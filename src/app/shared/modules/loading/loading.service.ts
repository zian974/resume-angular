...
export class LoadingService {
  ...
  protected loadingStates = new Map<LoadingContext, Map<LoaderId, boolean>>();

  // Both loading state maps are kept in-sync such that
  // they can be used by both sync and async methods.
  protected loadingStates$ = new Map<LoadingContext, Map<LoaderId, BehaviorSubject<boolean>>>();
  ...
  globalLoading(): boolean {
    const loaderStates = this.loadingStates.get(this);
  ...
  isLoading(context: LoadingContext, loaderId?: LoaderId): boolean {
    ...
    if (!loaderStates) {
      ...
      // console.log("isLoading", [...loaderStates.values()].filter(state => state).length > 0)
  ...
  isLoading$(context: LoadingContext, loaderId?: LoaderId): Observable<boolean> {
    ...
    if (!this.hasLoadingStates(context, coalescedLoaderId)) {
      this.setLoadingState(context, false, loaderId as LoaderId);
    }
    return this.loadingStates$.get(context)?.get(coalescedLoaderId) as Observable<boolean>;
  }
  ...
  clearLoadings(): void {
    this.loadingStates = new Map<LoadingContext, Map<LoaderId, boolean>>();
    this.loadingStates$ = new Map<LoadingContext, Map<LoaderId, BehaviorSubject<boolean>>>();
  }
  ...
  protected setLoadingState(context: LoadingContext, state: boolean, loaderId: LoaderId): void {
    ...
    if (!this.hasLoadingStates(context, loaderId)) {
      ...
      //     val.set(k,v);
      //   });
      //   val.set(loaderId, state);
      //   loadingStates.set(key, val);
      // });
